 ...
 */
public class TopicAnalysisActivity extends Activity {
	...
	private int topicMasterId, subjectMasterId;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (getIntent().hasExtra(IntentExtras.TEST_TYPE) && getIntent().hasExtra(IntentExtras.DIFFICULTY_LEVEL)) {
			...
			topicMasterId = getIntent().getIntExtra(IntentExtras.TOPIC_MASTER_ID, 0);
			subjectName = getIntent().getStringExtra(IntentExtras.SUBJECT_NAME);
			topicName = getIntent().getStringExtra(IntentExtras.TOPIC_NAME);
		...
		for (Test test : testList) {
			try {
				if (topicMasterId == 0) {
					...
					if (diffLevel == 0) {
						questions.addAll(dbUtils.getDatabaseHelper().getQuestionRuntimeExceptionDao().queryBuilder().where().eq("idTestLogMaster", (int) test.getIdTestLogMaster()).and().eq("idTopicMaster", topicMasterId).query());
					} else {
						questions.addAll(dbUtils.getDatabaseHelper().getQuestionRuntimeExceptionDao().queryBuilder().where().eq("idTestLogMaster", (int) test.getIdTestLogMaster()).and().eq("difficultyLevel", (int) diffLevel).and().eq("idTopicMaster", topicMasterId).query());
					}
